 ...
 *
 * The influencer play: a creator (say @chloe) tells their audience to enter
 * their code during onboarding. iOS writes it to
 * `users/{uid}.acquisition.creatorCode`. This route finds those users and
 * extends each one's Pro comp window by N days, reusing the same
 * `proAccessUntil` field the iOS app (2.3+) already reads — so it lights up
 * every Pro feature with no app update.
 *
 * IDEMPOTENT by design: each granted user is stamped in
 * `proCodeGrants[<code>]`. Re-running the same code skips anyone already
 * granted for it and only rewards NEW signups — so you can click it again as
 * the creator's numbers grow, without stacking weeks on the same people.
 *
...
const MAX_DAYS = 365;
// Matches iOS AcquisitionSource.normalizeCreatorCode: strip @, lowercase,
// trim. Also strips accents so "chloé" and "chloe" fold to one bucket.
function normalizeCode(raw: string): string {
  return raw
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "") // strip combining accents (chloé → chloe)
    .replace(/@/g, "")
    .trim()
    .toLowerCase();
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    // Find users whose acquisition.creatorCode matches. iOS normalizes on
    // write, but normalize both sides so legacy/odd values still match.
    ...
    snap.forEach((doc) => {
      ...
      matched.push({
        ...
        alreadyGranted: Object.prototype.hasOwnProperty.call(grants, code),
      });
    });
    ...
    // Extend each user's window from max(now, existing proAccessUntil).
      ...
      await ref.set(
        {
          ...
          [`proCodeGrants.${code}`]: FieldValue.serverTimestamp(),
        },
        { merge: true }
      );
